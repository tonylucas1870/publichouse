/*
  # Fix Anonymous Users Implementation

  1. Tables
    - Create anonymous_users table with proper structure
    - Add anonymous_user_id reference to findings table
  
  2. Functions
    - Create get_or_create_anonymous_user function with proper column references
    - Create convert_to_full_account function for upgrading anonymous users
...
ADD COLUMN IF NOT EXISTS anonymous_user_id uuid REFERENCES anonymous_users(id);

-- Enable RLS
ALTER TABLE anonymous_users ENABLE ROW LEVEL SECURITY;

-- Create function to manage anonymous users
...
BEGIN
  ...
  SELECT au.id, au.changeover_ids 
  INTO v_user_id, v_changeover_ids
  ...
  IF v_user_id IS NULL THEN
    ...
    INTO v_user_id, v_name, v_anonymous_id, v_changeover_ids;
  ELSE
...
GRANT ALL ON TABLE anonymous_users TO authenticated;
GRANT ALL ON TABLE anonymous_users TO service_role;
GRANT EXECUTE ON FUNCTION get_or_create_anonymous_user TO anon, authenticated;
GRANT EXECUTE ON FUNCTION convert_to_full_account TO authenticated;